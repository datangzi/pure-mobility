...
def load_network_data():
    if not os.path.exists(nodes_path):
        raise FileNotFoundError(f"nodes_osm.csv not found at {nodes_path}")
    if not os.path.exists(streets_path):
        raise FileNotFoundError(f"streets_osm.csv not found at {streets_path}")
...
# Time parameters
times = range(0, 101)  # t=0 to t=100 inclusive
